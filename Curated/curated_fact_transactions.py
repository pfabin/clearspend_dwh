...
def main():
    ...
    cur.execute("""
        DROP TABLE IF EXISTS curated.fact_transactions;
    """)
    conn.commit()
    ...
    """)
    conn.commit()
    ### Surrogate key (transaction_key) auto-generated by PostgreSQL
    ### FK columns use _key suffix to reference dimension surrogates
    ### merchant_id kept as passthrough (no dim_merchant table)

    # ==============================
    # Insert fact rows with dimension lookups
    # ==============================
    cur.execute("""
    ...
    """)
    conn.commit()
    ### LEFT JOINs used so that fact rows are preserved even if a dimension lookup fails (NULL FK)
    ### IS NOT DISTINCT FROM used for nullable columns (treats NULL = NULL as true)
