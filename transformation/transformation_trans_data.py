...
def main():
    ...
    cur.execute(f"""
        CREATE TABLE transformation.{table_name} (
            ...
            error_client_message VARCHAR(100)
        );
    """)
    ...
    for df in pd.read_sql_query(query, conn, chunksize=chunk_size):
        ...
        ### Creates merchant_country column, and if it is a state, labels it as "United States".
        ...
        # 5. is_error_tech + is_error_client + error_client_message
        ...
        df["is_error_client"] = ((df["errors"] != "") & (~df["errors"].str.fullmatch("Technical Glitch")))
        ### In the case of a client-end error, it is flagged for error rate analysis
        ...
        df["error_client_message"] = df["errors"].apply(extract_other_errors)
        df.drop(columns=["errors"], inplace=True)
        ### Adds the client errors to error_client_message and drops original column
        ...
        df = df[
            [
                ...
                "is_error_tech", "is_error_client", "error_client_message"
            ]
        ]
        ...
        with cur.copy(f"""
            ...
            (id, date_id, client_id, card_id, amount, is_refund,
             ...
             zip, mcc, is_error_tech, is_error_client, error_client_message)
