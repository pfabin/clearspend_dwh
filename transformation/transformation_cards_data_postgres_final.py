...
def main():
    ...
    ### Truncating everything after the decimal point, then converting to string
    ...
    df["credit_limit"] = df["credit_limit"].apply(
        lambda value: abs(value) if pd.notna(value) else pd.NA
    )
    ### Standardising the credit limit and converting negative numeric values to positive, assuming all is USD
